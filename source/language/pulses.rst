...
They are typically generated by automatic calibration routines the are periodically
run on the target system, which is in turn bootstrapped from previous calibrations.
The majority of OpenQASM users will default to using the default calibrations, however,
for those that want more control, but do not want to bootstrap calibrations for an entire
device it is expected that the target system provider will provide a ``backend.inc`` include
...
existing ones by using the same ``channel`` and ``frame`` as the existing ones in the
``backend.inc`` file. The example below demonstrates this in practice for a two-qubit,
...
.. code-block:: c
   ...
   defcal cx $1, $0 {
      ...
      sx $0; sx $1;
      barrier $0, $1;
      play(q1, CR90p, copyframe(q1_frame));
      barrier $0, $1;
      sx $0;
      sx $0;
      barrier $0, $1;
      rz(-pi/2) $0; rz(pi/2) $1;
      sx $0; sx $1;
      play(q1, CR90m, copyframe(q1_frame));
   }

The user would then include the ``backend.inc`` in their program and use them as demonstrated below
