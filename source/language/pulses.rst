...
As calibration grammars may require the ability to insert top-level configuration information, perform program setup,
or make inline calls to calibration-level instructions, OpenQASM supports the ability to declare a ``cal`` block.
Within the ``cal`` block, the semantics of the selected calibration grammar are valid.

The ``cal`` block is of the same scope level as the enclosing block, and the calibration grammar implementer
may choose to allow referencing of `const` values from outside of the ``cal`` block so long as these values are
in the enclosing block's scope. However, values may not leak back out to the enclosing blocks scope. In other words,
values declared within the ``cal`` block are only referenceable from other ``cal`` blocks or ``defcal`` declarations
that may observe that scope as defined by the calibration grammar implementer.

In practice, calibration grammars such as OpenPulse may apply a global scope to all identifiers in order to declare
values shared across all ``defcal`` calls thereby linking them together.
...
.. code-block:: c
   ...
   cal {
      ...
      // declare global frame; also reference `freq` variable from enclosing blocks scope
   ...
   defcal x $0 {
      ...
      play(d0, xp, d0f);
      // OpenPulse applies global scope to all identifiers so `new_freq` is accessable here
      frame.frequency = new_freq;
      play(d0, xp, d0f);
   }


Restrictions on cal and defcal bodies
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The contents of ``cal`` and ``defcal`` bodies are subject to the restriction they must have a definite duration
...
In addition, there is to be no control flow in the ``defcal``, as control-flow is natively supported
in OpenQASM.
