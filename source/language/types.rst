...
declares a reference to a quantum bit. These qubits are referred to as "program qubits" (in distinction to "physical qubits" on actual hardware; see below.) The statement ``qreg name[size];`` or ``qubit name[size];`` declares a
