...
def check_astring(astring, nqubits):
    ...
    if len(astring) > nqubits:
        raise Exception("The length of the hidden string is longer than the number of qubits")
    else:
...
def gen_bv_main(nQubits, hiddenString):
    ...
    #Apply Hadamard gates to the first (nQubits - 1) before querying the oracle
    for i in range(nQubits-1):
        bvCircuit.h(qr[i])

    #Apply 1 and Hadamard gate to the last qubit for storing the oracle's answer
...
from common import draw_circuit, print_qasm

def main(nQubits, hiddenString, prob, draw, outname):
    if hiddenString is None:
        hiddenString = generate_astring(nQubits-1, prob)
    assert check_astring(hiddenString, nQubits-1) is True, "Invalid hidden string"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate qasm of the Bernstein-Vazirani algorithm.")
