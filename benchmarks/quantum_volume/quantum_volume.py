...
def build_model_circuits(name, n, depth, num_circ=1):
    ...
    for i in range(num_circ):
        ...
        circuit_i = qp.create_circuit(name, [q], [c])
        # For each layer
        ...
        qp.add_circuit(name, circuit_i + meas)
    return qp
...
def main():
    ...
    qp = build_model_circuits(name=circuit_name, n=args.qubits,
    ...
    if args.run:
      ...
      ret = qp.execute(circuit_name, backend=backend, shots=1,
      ...
      sim_result = ret.get_counts(circuit_name)
      print(sim_result)
      return
