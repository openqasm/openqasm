...
class QsharpExecutor(object):
  ...
  def generator(self,filename):
    ...
    qs = open("backends/workspace/Qsharp/benchmark/benchmark.qs", 'w')
    name = "Bench"
    operation = name
  ...
  def run_simulation(self, filename):
    ...
    ret = subprocess.check_output("dotnet run -c Release", shell=True, cwd="backends/workspace/Qsharp/benchmark")
    return float(ret);
